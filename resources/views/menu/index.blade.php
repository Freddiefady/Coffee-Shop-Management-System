                                ...
                                <div class="row">
                                    @foreach ($drinks as $drink)
                                        <div class="col-md-4 text-center">
                                            <div class="menu-wrap">
                                                <a href="{{ route('product-single', $drink->name) }}" class="menu-img img mb-4"
                                                    style="background-image: url({{ asset('assets/images/'. $drink->image) }});"></a>
                                                <div class="text">
                                                    <h3><a href="{{ route('product-single', $drink->name) }}">
                                                        {{ $drink->name }}</a></h3>
                                                    <p>{{ $drink->description }}</p>
                                                    <p class="price"><span>${{ $drink->price }}</span></p>
                                                    <p><a href="{{ route('product-single', $drink->name) }}"
                            ...
                            <div class="tab-pane fade" id="v-pills-3" role="tabpanel" aria-labelledby="v-pills-3-tab">
                                <div class="row">
                                    @foreach ($desserts as $dessert)
                                        <div class="col-md-4 text-center">
                                            <div class="menu-wrap">
                                                <a href="{{ route('product-single', $dessert->name) }}" class="menu-img img mb-4"
                                                    style="background-image: url({{ asset('assets/images/'. $dessert->image) }});"></a>
                                                <div class="text">
                                                    <h3><a href="{{ route('product-single', $dessert->name) }}">{{ $dessert->name }}</a></h3>
                                                    <p>{{ $dessert->description }}</p>
                                                    <p class="price"><span>${{ $dessert->price }}</span></p>
                                                    <p><a href="{{ route('product-single', $dessert->name) }}"
                                                        class="btn btn-primary btn-outline-primary">Add to cart</a></p>
                                                </div>
                                            </div>
                                        </div>
